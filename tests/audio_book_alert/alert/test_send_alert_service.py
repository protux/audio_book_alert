...
def test_send_alert_sends_alert(
    ...
    send_alert_service.telegram_subscription_repository.get_all_subscribers_chat_ids.return_value = subscriber_chat_ids

    send_alert_service.send_alert(['Hallo welt'])

    send_alert_service.telegram_bot.send_message.assert_called_with(serialized_audio_books, subscriber_chat_ids)
...
def patched_serialize_all_audio_books_human_readable(monkeypatch) -> None:
    monkeypatch.setattr(alert_serializer, 'serialize_all_audio_books_human_readable', lambda x: ['book'])
